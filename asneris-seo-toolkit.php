...
add_action('admin_notices', 'asnerisseo_check_memory_limit');

function asnerisseo_check_memory_limit() {
    ...
    if ( ! $memory_limit || $memory_limit === '-1' ) {
        return; // unlimited or unknown
    }

    $bytes = asnerisseo_hr_to_bytes($memory_limit);

    if ( $bytes < 268435456 ) { // 256MB
        echo '<div class="notice notice-warning is-dismissible">
            <p>
                <strong>Asneris SEO Toolkit:</strong>
                Your PHP memory limit is <code>' . esc_html($memory_limit) . '</code>.
                We recommend at least <strong>256MB</strong> for optimal performance.
            </p>
        </div>';
    }
}

function asnerisseo_hr_to_bytes( $value ) {
    $value = trim( $value );
