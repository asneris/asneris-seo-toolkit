...
<div class="ASNERISSEO-card">
	...
	<tbody>
		...
		<td>
		<?php
		echo esc_html( $ASNERISSEO_sitemap_status['found'] ? 'Found: ' : 'Not Found: ' );
		echo esc_html( $ASNERISSEO_sitemap_status['url'] );
		?>
		</td>
		</tr>
		<tr>
		...
		<td>
		<?php
		echo esc_html( $ASNERISSEO_sitemap_status['in_robots'] ? 'Referenced in robots.txt - ' : 'Not found in robots.txt - ' );
		echo esc_html( $ASNERISSEO_sitemap_status['robots_message'] );
		?>
		</td>
		</tr>
		<tr>
...
<div class="ASNERISSEO-card">
	...
	<tbody>
		...
		<?php foreach ( array( 'title', 'description', 'canonical', 'robots', 'schema' ) as $type ) : ?>
		<tr>
...
<div class="ASNERISSEO-card">
	...
	$ASNERISSEO_indexing_warnings = array();
	...
	$ASNERISSEO_robots_response = wp_remote_get(
		$ASNERISSEO_robots_url,
		array(
			'timeout'   => 5,
			'sslverify' => true,
		)
	...
	if ( ! is_wp_error( $ASNERISSEO_robots_response ) && wp_remote_retrieve_response_code( $ASNERISSEO_robots_response ) === 200 ) {
		...
		$ASNERISSEO_blocked_sections = array();
	...
	} elseif ( $ASNERISSEO_sitemap_check['found'] ) {
		$ASNERISSEO_indexing_warnings[] = array(
			'check'   => 'Sitemap Accessibility',
			'status'  => 'pass',
	...
	$ASNERISSEO_test_urls    = array( home_url( '/' ) );
	$ASNERISSEO_recent_posts = get_posts(
		array(
			'numberposts' => 3,
			'post_status' => 'publish',
		)
	...
	foreach ( $ASNERISSEO_test_urls as $ASNERISSEO_url ) {
		$ASNERISSEO_response = wp_remote_head(
			$ASNERISSEO_url,
			array(
				'timeout'     => 5,
				'redirection' => 0,
		...
		if ( ! is_wp_error( $ASNERISSEO_response ) ) {
			...
			if ( in_array( $ASNERISSEO_code, array( 301, 302, 307, 308 ) ) ) {
...
<div class="ASNERISSEO-card">
	...
	global $wpdb;
	$ASNERISSEO_canonical_checks    = array();
	...
	if ( false === $ASNERISSEO_pages_to_home ) {
	  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Caching implemented with wp_cache_get/set above
		$ASNERISSEO_pages_to_home = $wpdb->get_var(
			$wpdb->prepare(
				"
      SELECT COUNT(*) 
      ...
      AND p.ID != %d
    ",
				$ASNERISSEO_home_url_normalized,
	...
	} elseif ( $ASNERISSEO_pages_to_home > 0 ) {
		$ASNERISSEO_canonical_checks[] = array(
			'check'   => 'Pages Canonicalizing to Homepage',
			'status'  => 'pass',
  ...
  ",
	...
	$ASNERISSEO_canonical_map = array();
	...
	} elseif ( ! empty( $ASNERISSEO_sampled_urls ) ) {
		$ASNERISSEO_canonical_checks[] = array(
			'check'   => 'Canonicals to Redirected URLs',
			'status'  => 'pass',
  ...
  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Simple count query, caching not needed
	$ASNERISSEO_mixed_protocol = $wpdb->get_var(
		$wpdb->prepare(
			"
    SELECT COUNT(*) 
    FROM {$wpdb->postmeta} pm
