...
<div class="cfseo-function-group">
  <div class="cfseo-group-header" data-group="indexing">
    <div class="cfseo-group-title">
      ...
      <h3>🚦 <?php _e('Indexing Control', 'cfseo'); ?></h3>
    ...
    <div class="cfseo-group-summary">
      ...
      <span><?php echo $indexing_pass; ?> / <?php echo $indexing_total; ?> <?php _e('passed', 'cfseo'); ?></span>
  ...
  <div class="cfseo-group-content" id="group-indexing" style="display: none;">
    <p class="cfseo-group-description">
      <?php _e('Controls whether search engines can index this page', 'cfseo'); ?>
    </p>
    
    <!-- Robots Meta Tag -->
    <div class="cfseo-check-item">
      <div class="cfseo-check-header">
        <strong><?php _e('Robots Meta Tag', 'cfseo'); ?></strong>
      ...
      <?php if (count($results['robots']) === 1): ?>
        <div class="cfseo-check-details">
          ...
          <p class="description" style="color: #dba617;">
            ⚠ <?php _e('Page has indexing restrictions:', 'cfseo'); ?>
            <?php if ($has_noindex): ?><?php _e('NOINDEX', 'cfseo'); ?><?php endif; ?>
            <?php if ($has_nofollow): ?><?php _e('NOFOLLOW', 'cfseo'); ?><?php endif; ?>
          </p>
          <?php else: ?>
          <p class="description">✓ <?php _e('Page is indexable', 'cfseo'); ?></p>
          <?php endif; ?>
        </div>
      <?php elseif (count($results['robots']) === 0): ?>
        <p class="description">✓ <?php _e('No robots meta (indexable by default)', 'cfseo'); ?></p>
      <?php else: ?>
        <p class="description" style="color: #d63638;">
          ✗ <?php _e('Multiple robots meta tags found', 'cfseo'); ?>:
      ...
      <details class="cfseo-technical-details">
        ...
        <div class="cfseo-tech-box">
          ...
          <p><strong><?php _e('PURPOSE:', 'cfseo'); ?></strong> <?php _e('Controls indexing and link following behavior', 'cfseo'); ?></p>
          <p><strong><?php _e('IMPACT:', 'cfseo'); ?></strong> <?php _e('Direct (critical) - Prevents indexing if noindex is set', 'cfseo'); ?></p>
          <p><strong><?php _e('VALIDATION:', 'cfseo'); ?></strong> XPath <code>//meta[@name="robots"]</code></p>
          <p><strong><?php _e('EXPECTED:', 'cfseo'); ?></strong> <?php _e('0 or 1 tag, no conflicting directives', 'cfseo'); ?></p>
        </div>
      </details>
    </div>
    ...
    <div class="cfseo-check-item">
      <div class="cfseo-check-header">
        <strong><?php _e('Canonical URL', 'cfseo'); ?></strong>
      ...
      <div class="cfseo-check-details">
        <?php if (count($results['canonical']) === 1): ?>
          ...
          <?php if ($canonical_check['status'] === 'pass'): ?>
            <p class="description">✓ <?php _e('Canonical URL is valid (HTTP 200, indexable)', 'cfseo'); ?></p>
          <?php else: ?>
            <p class="description" style="color: #d63638;">
        ...
        <?php elseif (count($results['canonical']) === 0): ?>
          <p class="description" style="color: #dba617;">⚠ <?php _e('No canonical URL found', 'cfseo'); ?></p>
        <?php else: ?>
          <p class="description" style="color: #d63638;">
            ✗ <?php _e('Multiple canonical URLs found', 'cfseo'); ?>:
      ...
      <details class="cfseo-technical-details">
        ...
        <div class="cfseo-tech-box">
          ...
          <p><strong><?php _e('PURPOSE:', 'cfseo'); ?></strong> <?php _e('Tells search engines which URL is the authoritative version', 'cfseo'); ?></p>
          <p><strong><?php _e('IMPACT:', 'cfseo'); ?></strong> <?php _e('Direct (strong) - Consolidates signals for duplicate content', 'cfseo'); ?></p>
          <p><strong><?php _e('VALIDATION:', 'cfseo'); ?></strong> XPath <code>//link[@rel="canonical"]</code></p>
          <p><strong><?php _e('EXPECTED:', 'cfseo'); ?></strong> <?php _e('Exactly 1, URL must return HTTP 200, must be indexable (not noindex)', 'cfseo'); ?></p>
        </div>
      </details>
    </div>
  </div>
