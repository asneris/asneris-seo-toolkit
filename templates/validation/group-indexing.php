...
if ( count( $results['robots'] ) === 1 ) {
	++$ASNERISSEO_indexing_pass;
}
if ( $canonical_check['status'] === 'pass' ) {
	++$ASNERISSEO_indexing_pass;
}
?>
		...
		<div class="ASNERISSEO-check-details">
			...
			⚠ <?php esc_html_e( 'Page has indexing restrictions:', 'asneris-seo-toolkit' ); ?>
				...
				if ( $ASNERISSEO_has_noindex ) :
					?>
					<?php esc_html_e( 'NOINDEX', 'asneris-seo-toolkit' ); ?><?php endif; ?>
				<?php
				if ( $ASNERISSEO_has_nofollow ) :
					?>
					<?php esc_html_e( 'NOFOLLOW', 'asneris-seo-toolkit' ); ?><?php endif; ?>
			</p>
			<?php else : ?>
