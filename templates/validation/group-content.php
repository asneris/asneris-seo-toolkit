...
if ( $headings && $headings['has_h1'] ) {
	++$ASNERISSEO_content_pass;
}
if ( $images && $images['with_alt'] > 0 ) {
	++$ASNERISSEO_content_pass;
}
if ( $links && ( $links['internal'] > 0 || $links['external'] > 0 ) ) {
	++$ASNERISSEO_content_pass;
}
?>
<div class="ASNERISSEO-function-group">
	...
	<div class="ASNERISSEO-check-item">
		...
		<?php echo wp_kses_post( ASNERISSEO_Validation::get_status_badge( $headings && $headings['has_h1'] ? 1 : 0 ) ); ?>
		</div>
		<?php if ( $headings ) : ?>
	...
	<div class="ASNERISSEO-check-item">
		...
		<?php echo wp_kses_post( ASNERISSEO_Validation::get_status_badge( $images && $images['with_alt'] > 0 ? 1 : 0 ) ); ?>
		</div>
		<?php if ( $images ) : ?>
		<div class="ASNERISSEO-check-details">
			<p class="description">
			...
			<?php if ( $images['without_alt'] > 0 ) : ?>
				<br><span style="color: #dba617;">⚠ 
				<?php
				/* translators: %d: number of images missing alt text */
				printf( esc_html__( '%d images missing alt text', 'asneris-seo-toolkit' ), esc_html( $images['without_alt'] ) );
				?>
				</span>
			<?php endif; ?>
			</p>
	...
	<div class="ASNERISSEO-check-item">
		...
		<span class="ASNERISSEO-engine-scope"><?php esc_html_e( 'Engines: Google, Bing', 'asneris-seo-toolkit' ); ?></span>
		<?php echo wp_kses_post( ASNERISSEO_Validation::get_status_badge( $links && ( $links['internal'] > 0 || $links['external'] > 0 ) ? 1 : 0 ) ); ?>
		</div>
		<?php if ( $links ) : ?>
		<div class="ASNERISSEO-check-details">
			<p class="description">
