...
<div class="cfseo-card" style="margin-top: 20px;">
  <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
    <div>
      <h2 style="margin: 0;">📊 <?php _e('Overall SEO Score', 'cfseo'); ?></h2>
      <p style="margin: 5px 0 0 0; color: #646970;">
        <?php _e('Testing:', 'cfseo'); ?> 
  ...
  <div style="padding: 15px; background: #f6f7f7; border-radius: 4px; margin-bottom: 15px;">
    <strong><?php echo $score['passed']; ?> / <?php echo $score['total']; ?> <?php _e('checks passed', 'cfseo'); ?></strong>
  </div>
  ...
  <div style="background: #f0f6fc; border-left: 3px solid #2271b1; padding: 15px; margin-top: 15px;">
    <h3 style="margin: 0 0 10px 0; font-size: 15px; color: #2271b1;">
      💡 <?php _e('Understanding Your Score', 'cfseo'); ?>
    </h3>
    ...
    <div style="margin: 10px 0;">
      <?php if ($can_index): ?>
        <p style="margin: 5px 0; color: #00a32a;">
          <strong>✓ <?php _e('Your page CAN be indexed', 'cfseo'); ?></strong> 
          (<?php _e('HTTP 200 + no blocking rules', 'cfseo'); ?>)
        </p>
      <?php else: ?>
        <p style="margin: 5px 0; color: #d63638;">
          <strong>✗ <?php _e('Your page CANNOT be indexed', 'cfseo'); ?></strong>
        </p>
        <?php if ($critical['passed'] < $critical['total']): ?>
          <p style="margin: 5px 0; color: #d63638; font-size: 13px;">
            <?php _e('Fix critical issues below to allow search engines to index this page.', 'cfseo'); ?>
          </p>
        <?php endif; ?>
      <?php endif; ?>
      ...
      <?php if ($recommended['passed'] < $recommended['total']): ?>
        <p style="margin: 5px 0; color: #f0c33c;">
          <strong>⚠️ <?php printf(__('%d recommended items missing', 'cfseo'), $recommended['total'] - $recommended['passed']); ?></strong>
        </p>
        <p style="margin: 5px 0; font-size: 13px; color: #666;">
          <?php _e('These improve how your page appears in search results and gets discovered.', 'cfseo'); ?>
        </p>
      <?php endif; ?>
      
      <?php if ($optimization['passed'] < $optimization['total'] && $can_index): ?>
        <p style="margin: 5px 0; color: #666;">
          <strong>📈 <?php printf(__('%d optimization opportunities', 'cfseo'), $optimization['total'] - $optimization['passed']); ?></strong>
        </p>
        <p style="margin: 5px 0; font-size: 13px; color: #666;">
          <?php _e('Add social tags, schema markup, and verification codes for enhanced features.', 'cfseo'); ?>
        </p>
      <?php endif; ?>
    </div>
    
    <p style="margin: 10px 0 0 0; font-size: 12px; color: #666; font-style: italic; border-top: 1px solid #ddd; padding-top: 10px;">
      <?php _e('Scoring method: Critical checks (60%), Recommended (30%), Optimization (10%)', 'cfseo'); ?>
    </p>
  </div>
</div>
