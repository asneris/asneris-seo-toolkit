...
<div class="cfseo-function-group">
  <div class="cfseo-group-header" data-group="schema">
    <div class="cfseo-group-title">
      ...
      <h3>⭐ <?php _e('Rich Results (Structured Data)', 'cfseo'); ?></h3>
    ...
    <div class="cfseo-group-summary">
      ...
      <span><?php echo $schema_pass; ?> / <?php echo $schema_total; ?> <?php _e('passed', 'cfseo'); ?></span>
  ...
  <div class="cfseo-group-content" id="group-schema" style="display: none;">
    <p class="cfseo-group-description">
      <?php _e('Structured data that enables rich results in search engines', 'cfseo'); ?>
    </p>
    
    <!-- Schema JSON-LD -->
    <div class="cfseo-check-item">
      <div class="cfseo-check-header">
        <strong><?php _e('Schema.org JSON-LD', 'cfseo'); ?></strong>
      ...
      <div class="cfseo-check-details">
        <?php if (!empty($results['schema'])): ?>
          <p class="description">
            ✓ <?php printf(__('%d Schema block(s) found', 'cfseo'), count($results['schema'])); ?>
          </p>
          ...
          <details style="margin-top: 10px;">
            <summary><strong><?php printf(__('Block %d:', 'cfseo'), $idx + 1); ?></strong> <?php echo esc_html($type); ?></summary>
        ...
        <?php else: ?>
          <p class="description" style="color: #dba617;">
            ⚠ <?php _e('No Schema.org structured data found', 'cfseo'); ?>
          </p>
        <?php endif; ?>
      </div>
      <details class="cfseo-technical-details">
        ...
        <div class="cfseo-tech-box">
          ...
          <p><strong><?php _e('PURPOSE:', 'cfseo'); ?></strong> <?php _e('Enables rich results (reviews, events, recipes, etc.) in SERPs', 'cfseo'); ?></p>
          <p><strong><?php _e('IMPACT:', 'cfseo'); ?></strong> <?php _e('Indirect (medium-high) - Can improve CTR via rich snippets', 'cfseo'); ?></p>
          <p><strong><?php _e('VALIDATION:', 'cfseo'); ?></strong> XPath <code>//script[@type="application/ld+json"]</code>, <?php _e('JSON parse check', 'cfseo'); ?></p>
          <p><strong><?php _e('ALLOWED:', 'cfseo'); ?></strong> <?php _e('Multiple blocks are valid. Check for type conflicts (e.g., Product + Article)', 'cfseo'); ?></p>
          <p><strong><?php _e('CONFIDENCE LEVEL:', 'cfseo'); ?></strong> <?php _e('Medium - Basic syntax validation only. Use Google Rich Results Test for full validation.', 'cfseo'); ?></p>
        </div>
      </details>
    </div>
  </div>
