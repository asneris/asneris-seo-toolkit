...
class ASNERISSEO_404_Monitor {
	...
	public static function init() {
		...
		add_action( 'template_redirect', array( __CLASS__, 'capture_404_request' ), 20 );
		add_action( self::ANALYSIS_CRON_HOOK, array( __CLASS__, 'run_priority_analysis' ) );
	...
	public static function run_priority_analysis( $limit = 200, $force_manual = false, $restrict_ids = array() ) {
		global $wpdb;

		$limit        = max( 1, min( 1000, absint( $limit ) ) );
		$force_manual = ! empty( $force_manual );
		$restrict_ids = is_array( $restrict_ids ) ? array_values( array_filter( array_map( 'absint', $restrict_ids ) ) ) : array();

		$where_sql = 'WHERE status IN (%s, %s)';
		$params    = array( 'active', 'redirected' );
	...
	public static function enqueue_assets( $hook ) {
		if ( $hook !== 'asneris-seo-toolkit_page_' . ASNERIS_MENU_SLUG . '-404-monitor' ) {
			return;
		}
	...
	private static function table_exists( $table_name ) {
		...
		$cache_key = 'asnerisseo_table_exists_' . md5( $table_name );
		$cached_result = wp_cache_get( $cache_key, 'asnerisseo_404_monitor' );
		if ( false !== $cached_result ) {
			return (bool) $cached_result;
		}

		$previous_suppress = $wpdb->suppress_errors( true );
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Table existence probe is scoped to plugin-owned table names and cached for a short window.
		$probe             = $wpdb->get_var(
		...
		if ( null !== $probe ) {
			wp_cache_set( $cache_key, true, 'asnerisseo_404_monitor', 300 );
			return true;
		}

		if ( false !== strpos( $error, 'doesn\'t exist' ) || false !== strpos( $error, 'relation' ) || false !== strpos( $error, 'no such table' ) ) {
			wp_cache_set( $cache_key, false, 'asnerisseo_404_monitor', 300 );
			return false;
		}
	...
	public static function capture_404_request() {
		...
		$requested_url = esc_url_raw( home_url( add_query_arg( array(), $request_uri ) ) );
	...
	public static function get_logs( $args = array() ) {
		...
		$allowed_sort = array( 'last_seen', 'first_seen', 'hit_count', 'path', 'priority', 'redirect_target' );
		...
		if ( ! in_array( $status, array( 'active', 'ignored', 'redirected', 'fixed', 'all' ), true ) ) {
			$status = 'active';
		}
		...
		$allowed_recommendation_filters = array( 'all', 'proposed_redirect_url', 'hotfix_broken_url', 'find_redirect_page' );
		...
		if ( empty( $recommendation_filters ) || in_array( 'all', $recommendation_filters, true ) ) {
			$recommendation_filters = array( 'all' );
		}
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Query builder uses a prepared template with validated, whitelisted SQL fragments and plugin-owned table names.
		$total = (int) $wpdb->get_var(
			// phpcs:disable WordPress.DB.PreparedSQL.NotPrepared -- The count query is assembled from a whitelisted predicate set and the placeholders are bound via $wpdb->prepare().
			$wpdb->prepare(
				$count_sql,
				$params
			)
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Query builder uses a prepared template with validated, whitelisted SQL fragments and plugin-owned table names.
		$rows = $wpdb->get_results(
			// phpcs:disable WordPress.DB.PreparedSQL.NotPrepared -- The log query is assembled from a whitelisted predicate set and the placeholders are bound via $wpdb->prepare().
			$wpdb->prepare(
				$query_sql,
				$query_params
			),
		...
		$items = array_map( array( __CLASS__, 'sanitize_log_row' ), $rows );
	...
	public static function update_log( $id, $payload = array() ) {
		global $wpdb;
		...
		if ( ! in_array( $status, array( 'active', 'ignored', 'redirected', 'fixed', 'deleted' ), true ) ) {
	...
	public static function bulk_action( $ids, $action, $redirect_target = '' ) {
		...
		foreach ( $ids as $id ) {
			...
			if ( 'delete' === $action ) {
				...
				$ok = self::update_log( $id, array( 'status' => 'ignored' ) );
			} elseif ( 'activate' === $action ) {
				$ok = self::update_log( $id, array( 'status' => 'active' ) );
	...
	public static function export_logs( $args = array() ) {
		...
		$all_items   = is_array( $logs['items'] ?? null ) ? $logs['items'] : array();
		...
		if ( $total_pages > 1 ) {
			for ( $page = 2; $page <= $total_pages; $page++ ) {
				...
				$page_items   = is_array( $page_logs['items'] ?? null ) ? $page_logs['items'] : array();
				if ( ! empty( $page_items ) ) {
	...
	private static function upsert_log( $row ) {
	  ...
	  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Internal plugin analytics insert
		$wpdb->insert(
			...
			array( '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%s' )
		);
	}
	...
	private static function get_redirect_candidates_from_posts_pages() {
		...
		if ( ! is_array( $post_ids ) || empty( $post_ids ) ) {
			self::$redirect_candidates_cache   = array();
			self::$redirect_token_lookup_cache = array();
			return self::$redirect_candidates_cache;
		}
	...
	private static function build_redirect_token_lookup( $candidates ) {
		...
		foreach ( $candidates as $index => $candidate ) {
			$tokens = (array) ( $candidate['tokens'] ?? array() );
	...
	private static function score_redirect_candidate( $missing, $candidate ) {
		...
		$missing_tokens   = array_values( array_unique( array_map( 'sanitize_key', (array) ( $missing['tokens'] ?? array() ) ) ) );
		$candidate_tokens = array_values( array_unique( array_map( 'sanitize_key', (array) ( $candidate['tokens'] ?? array() ) ) ) );
		...
		$old_slugs = (array) ( $candidate['old_slugs'] ?? array() );
	...
	private static function is_search_referrer( $referrer ) {
		...
		$search_hosts = array( 'google.', 'bing.', 'yahoo.', 'duckduckgo.', 'yandex.', 'baidu.' );
	...
	private static function build_recommandation_text( $level, $analysis = array(), $redirect_target = '' ) {
		unset( $level );
