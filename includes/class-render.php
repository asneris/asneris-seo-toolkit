...
class ASNERISSEO_Render {
  ...
  public static function render_meta_tags() {
    ...
    $generated_title = get_post_meta($id, '_asneris_generated_title', true);
    $generated_description = get_post_meta($id, '_asneris_generated_description', true);
    ...
    if (empty($final_desc)) {
      if ( ! empty( $generated_description ) ) {
        $final_desc = $generated_description;
      }
    }
    if (empty($final_desc)) {
