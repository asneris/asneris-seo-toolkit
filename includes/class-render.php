...
class GSCSEO_Render {
  ...
  public static function render_meta_tags() {
    ...
    $google = GSCSEO_Admin_Settings::get('google_verification');
    if ($google) {
      echo '<meta name="google-site-verification" content="' . esc_attr($google) . '">' . "\n";
    }

    $bing = GSCSEO_Admin_Settings::get('bing_verification');
    if ($bing) {
      echo '<meta name="msvalidate.01" content="' . esc_attr($bing) . '">' . "\n";
    }

    $yandex = GSCSEO_Admin_Settings::get('yandex_verification');
    if ($yandex) {
    ...
    $desc = get_post_meta($id, '_gscseo_description', true);
    $canon = get_post_meta($id, '_gscseo_canonical', true) ?: get_permalink($id);
    $robots_index = get_post_meta($id, '_gscseo_robots_index', true) ?: 'index';
    $robots_follow = get_post_meta($id, '_gscseo_robots_follow', true) ?: 'follow';
    
    // Open Graph meta
    $og_title = get_post_meta($id, '_gscseo_og_title', true);
    $og_description = get_post_meta($id, '_gscseo_og_description', true);
    ...
    if (empty($final_desc)) {
      ...
      $template_desc = GSCSEO_Templates::generate_description($post);
    ...
    $final_og_image = $og_image ?: GSCSEO_Admin_Settings::get('default_og_image');
    ...
    // Facebook App ID
    $fb_app_id = GSCSEO_Admin_Settings::get('facebook_app_id');
    if ($fb_app_id) {
    ...
    $twitter_username = GSCSEO_Admin_Settings::get('twitter_username');
    if ($twitter_username) {
    ...
    $theme_color = GSCSEO_Admin_Settings::get('theme_color');
    if ($theme_color) {
