...
class ASNERISSEO_Diagnostics {
  ...
  public static function http_test_checks( $url ) {
    ...
    if (!is_wp_error($get_response)) {
      ...
      if (!empty($body)) {
        ...
        $primary_entity_priority = [ 'Article', 'BlogPosting', 'NewsArticle', 'Product', 'Service', 'Organization', 'FAQPage', 'WebPage' ];
        ...
        $canonical_valid = '' !== $canonical_url && (bool) wp_http_validate_url( $canonical_url );
        ...
        $checks[] = [
          ...
          'details' => 1 === $canonical_count ? $canonical_url : ( 0 === $canonical_count ? 'No canonical tag found.' : 'Multiple canonical tags found.' )
        ];
        $checks[] = [
          'label' => 'Canonical Valid URL',
          'status' => $canonical_valid ? 'pass' : 'warning',
          'result' => $canonical_valid ? 'Valid' : 'Invalid',
          'details' => '' !== $canonical_url ? $canonical_url : 'No canonical URL to validate.'
        ];
        $checks[] = [
          'label' => 'Self Canonical',
          'status' => ( $canonical_valid && $canonical_normalized === $current_normalized ) ? 'pass' : 'warning',
          'result' => ( $canonical_valid && $canonical_normalized === $current_normalized ) ? 'Yes' : 'No',
          'details' => '' !== $canonical_url ? 'Compared canonical to page URL.' : 'No canonical URL to compare.'
        ];
        ...
        $checks[] = [
          ...
          'details' => $canonical_valid ? 'HTTP check on canonical URL target.' : 'Skipped because canonical URL is invalid or missing.'
        ];
  ...
  private static function append_local_fallback_checks( $url, array &$checks, $network_error_message = '' ) {
    ...
    if ( '' === $robots_follow || ! in_array( $robots_follow, [ 'follow', 'nofollow' ], true ) ) {
    ...
    $checks[] = [
      ...
      'result' => '' !== $canonical ? 'Present' : 'Missing',
      'details' => '' !== $canonical ? $canonical : 'No local canonical URL configured.'
    ];

    $checks[] = [
      'label' => 'Canonical Valid URL',
      'status' => ( '' !== $canonical && wp_http_validate_url( $canonical ) ) ? 'pass' : 'warning',
      'result' => ( '' !== $canonical && wp_http_validate_url( $canonical ) ) ? 'Valid' : 'Invalid',
      'details' => '' !== $canonical ? $canonical : 'No canonical URL to validate.'
    ];

    $checks[] = [
      'label' => 'Self Canonical',
      'status' => '' === $canonical ? 'warning' : ( $canonical_normalized === $input_normalized ? 'pass' : 'warning' ),
      'result' => '' === $canonical ? 'Unknown' : ( $canonical_normalized === $input_normalized ? 'Self-referencing' : 'Different target' ),
      'details' => '' === $canonical ? 'Cannot verify self-reference without canonical.' : 'Compared input URL against canonical URL.'
    ];
