...
class ASNERISSEO_Dashboard {
	...
	public static function get_dashboard_summary_payload() {
		...
		return array(
			...
			'links'        => array(
				...
				'settingsAiSearchability' => admin_url( 'admin.php?page=' . ASNERIS_MENU_SLUG . '-settings&tab=aiSearchability' ),
	 ...
	 */
	private static function enqueue_react_assets( $debug_info = array() ) {
	...
	public static function enqueue_assets( $hook ) {
		...
		if ( ! self::is_dashboard_hook( $hook ) ) {
			return;
		}
	...
	private static function get_config_status() {
		...
		$llms_settings            = get_option( 'asneris_llms_settings', array() );
		$llms_state               = get_option( 'asneris_llms_state', array() );
		$llms_enabled             = ! empty( $llms_settings['enabled'] );
		$llms_has_content         = ! empty( $llms_state['draft_content'] ) || ! empty( $llms_state['published_content'] );
		...
		return array(
			...
			'aiSearchability'           => array(
				'label'     => 'AI Searchability',
				'icon'      => 'dashicons-search',
				'completed' => $llms_enabled && $llms_has_content,
				'items'     => array(
					'AI Searchability enabled'            => $llms_enabled,
					'llms.txt draft or publish available' => $llms_has_content,
				),
			),
			...
			'page_diagnostics_settings' => array(
	...
	private static function get_diagnostic_summary() {
		...
		if ( false === $sitemap_exists ) {
			$sitemap_exists = 0;
			$sitemap_urls   = array( home_url( '/wp-sitemap.xml' ), home_url( '/sitemap.xml' ) );
		...
		// Check if robots.txt is reachable (cached for 1 hour).
		// This supports virtual robots.txt served by WordPress when no physical file exists.
		$robots_txt_exists = get_transient( 'ASNERISSEO_robots_txt_exists' );
		if ( false === $robots_txt_exists ) {
			$robots_txt_exists = 0;
			$robots_response   = wp_remote_head( home_url( '/robots.txt' ), array( 'timeout' => 3 ) );
			if ( ! is_wp_error( $robots_response ) && wp_remote_retrieve_response_code( $robots_response ) === 200 ) {
				$robots_txt_exists = 1;
			}
			set_transient( 'ASNERISSEO_robots_txt_exists', $robots_txt_exists, HOUR_IN_SECONDS );
		}
		$robots_txt_exists = (bool) $robots_txt_exists;
	...
	private static function get_redirect_count() {
		global $wpdb;
		$table_name = self::sanitize_table_name_for_query( $wpdb->prefix . 'ASNERISSEO_redirects' );

		if ( '' === $table_name ) {
			return 0;
		}

		// Check if table exists using a prepared table probe with caching.
		$table_exists_cache_key = 'ASNERISSEO_redirect_table_exists';
		$table_exists           = wp_cache_get( $table_exists_cache_key );

		if ( false === $table_exists ) {
			$previous_suppress = $wpdb->suppress_errors( true );
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- The table probe is intentionally cached and scoped to the plugin's custom table name.
			$probe        = $wpdb->get_var(
				$wpdb->prepare( 'SHOW TABLES LIKE %s', $table_name )
			);
			$table_exists = null !== $probe;
		...
		if ( false === $count ) {
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- This custom-table count is cached and uses a prepared statement.
