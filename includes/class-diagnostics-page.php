...
class ASNERISSEO_Diagnostics_Page {
	...
	public static function enqueue_assets( $hook ) {
		...
		if ( $hook !== 'asneris-seo-toolkit_page_' . ASNERIS_MENU_SLUG . '-diagnostics' ) {
			return;
		}
		...
		if ( file_exists( $react_asset_path ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'defaultOgImage'                 => esc_url_raw( ASNERISSEO_Admin_Settings::get( 'default_og_image' ) ),
					'default_og_image'               => esc_url_raw( ASNERISSEO_Admin_Settings::get( 'default_og_image' ) ),
					...
					'mountSelector'                  => '.asnerisseo-fallback-page-diagnostics',
					'hideFallback'                   => true,
				)
			);
		}
		...
		// Keep fallback diagnostics table helpers scoped so React tables are not constrained.
		$inline_css = '.asnerisseo-fallback-page-diagnostics table td { word-wrap: break-word; word-break: break-word; overflow-wrap: break-word; }
  .asnerisseo-fallback-page-diagnostics table td code { display: inline-block; max-width: 100%; word-break: break-all; white-space: normal; background: #f0f0f1; padding: 2px 6px; border-radius: 3px; }
  .asnerisseo-fallback-page-diagnostics table { table-layout: fixed; width: 100%; }
  .asnerisseo-fallback-page-diagnostics table th:first-child, .asnerisseo-fallback-page-diagnostics table td:first-child { width: 250px; }
  .asnerisseo-fallback-page-diagnostics table th:nth-child(2), .asnerisseo-fallback-page-diagnostics table td:nth-child(2) { width: 120px; }';
	...
	private static function analyze_url( $url ) {
		...
		if ( ! wp_http_validate_url( $url ) ) {
			return array( 'error' => 'Invalid URL provided' );
		}
		...
		if ( ! $host || ! $site_host || strtolower( $host ) !== strtolower( $site_host ) ) {
			return array( 'error' => 'Security: Only URLs from this site can be analyzed' );
		}

		// Rate limiting: Prevent spam requests
		$throttle_key = 'asneris_analyze_' . md5( $url );
		if ( get_transient( $throttle_key ) ) {
			return array( 'error' => 'Too many requests. Please wait 5 seconds before analyzing this URL again.' );
		}
		...
		if ( strpos( $content_type, 'text/html' ) === false ) {
			return array( 'error' => 'Not an HTML document (Content-Type: ' . esc_html( $content_type ) . ')' );
		}
		...
		if ( strlen( $html ) > 2 * 1024 * 1024 ) {
			return array( 'error' => 'Response too large (max 2MB)' );
		}
		...
		foreach ( $canonicals as $canonical ) {
			...
			if ( ! empty( $href ) ) {
				...
				if ( ! is_wp_error( $canon_response ) ) {
						$results['canonical_target_status'][ $href ] = array(
	...
	<div class="wrap ASNERISSEO-admin-wrap">
		...
		<div class="asnerisseo-fallback-page-diagnostics">
		<h1>
		...
		<div class="ASNERISSEO-card">
			...
			<tbody>
				...
				<td>
					...
					<?php
					if ( $code == 200 ) {
						echo 'Indexable (Success)';
					} elseif ( $code >= 300 && $code < 400 ) {
						echo 'Redirect (May affect indexing)';
					} elseif ( $code >= 400 && $code < 500 ) {
						echo 'Not indexable (Client error)';
					} elseif ( $code >= 500 ) {
						echo 'Not indexable (Server error)';
					}
					?>
				</td>
				</tr>
		...
		<div class="ASNERISSEO-card">
			...
			<details style="margin-top: 15px;">
				...
				<pre style="background: #f6f7f7; padding: 10px; border-left: 3px solid #2271b1; overflow-x: auto; overflow-wrap: break-word; white-space: pre-wrap; word-break: break-all; font-size: 12px; max-width: 100%;">
					<?php
		...
		</div><!-- .asnerisseo-fallback-page-diagnostics -->
	</div><!-- .wrap -->
