...
class ASNERISSEO_Bulk_Edit {
	...
	public static function enqueue_assets( $hook ) {
		...
		if ( $hook !== 'asneris-seo-toolkit_page_' . ASNERIS_MENU_SLUG . '-bulk-edit' ) {
			return;
		}
		...
		if ( file_exists( $react_asset_path ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'mountSelector'                  => '.asnerisseo-fallback-bulk-edit',
					'hideFallback'                   => true,
				)
			);
		}
		...
		wp_enqueue_style( 'asnerisseo-bulk-edit', ASNERISSEO_URL . 'assets/css/admin-style.css', array(), $css_version );
		wp_enqueue_script( 'asnerisseo-bulk-edit', ASNERISSEO_URL . 'assets/js/bulk-edit.js', array( 'jquery' ), $js_version, true );

		wp_localize_script(
			'asnerisseo-bulk-edit',
			'asnerisBulkEdit',
			array(
	...
	public static function render_page() {
		...
		if ( $form_nonce_ok ) {
			$allowed_keys = array( 'seo_title', 'seo_description', 'robots_index' );
	...
	<div class="wrap ASNERISSEO-admin-wrap">
		...
		<div class="asnerisseo-fallback-bulk-edit">
		<h1>
			...
			<table id="ASNERISSEO-bulk-edit-table" class="wp-list-table widefat striped">
				...
				<?php if ( $posts_query->have_posts() ) : ?>
					...
					while ( $posts_query->have_posts() ) :
						...
						$title_placeholder = $title_preview ? sprintf( __( 'Auto (Using default template): %s', 'asneris-seo-toolkit' ), $title_preview ) : __( 'Leave blank for auto-generated title', 'asneris-seo-toolkit' );
						/* translators: %s is the auto-generated value from the template system */
						$desc_placeholder = $desc_preview ? sprintf( __( 'Auto (Using default template): %s', 'asneris-seo-toolkit' ), $desc_preview ) : __( 'Leave blank for auto-generated description', 'asneris-seo-toolkit' );
						?>
					<tr>
					<td class="check-column col-checkbox">
	...
	<div id="ASNERISSEO-confirm-modal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:100000; align-items:center; justify-content:center;">
		...
		</div><!-- .asnerisseo-fallback-bulk-edit -->
	</div>
	...
	public static function ajax_bulk_save() {
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied', 'asneris-seo-toolkit' ) ) );
			return;
		}
	  ...
	  // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Intentionally accessing raw values to detect dangerous patterns before sanitization strips them
		$titles_raw = isset( $_POST['seo_title'] ) ? map_deep( wp_unslash( $_POST['seo_title'] ), 'strval' ) : array();
	  // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Intentionally accessing raw values to detect dangerous patterns before sanitization strips them
		$descriptions_raw = isset( $_POST['seo_description'] ) ? map_deep( wp_unslash( $_POST['seo_description'] ), 'strval' ) : array();
	  // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Intentionally accessing raw values to detect dangerous patterns before sanitization strips them
		$robots_raw = isset( $_POST['robots_index'] ) ? map_deep( wp_unslash( $_POST['robots_index'] ), 'strval' ) : array();

		if ( empty( $post_ids ) ) {
			wp_send_json_error( array( 'message' => __( 'Please select at least one post to update.', 'asneris-seo-toolkit' ) ) );
			return;
		}
		...
		foreach ( $post_ids as $post_id ) {
			...
			if ( isset( $robots_raw[ $post_id ] ) && ! empty( $robots_raw[ $post_id ] ) ) {
				...
				if ( ! in_array( $original_robots, array( 'index', 'noindex' ), true ) ) {
					$validation_errors[] = sprintf(
		...
		$titles       = isset( $_POST['seo_title'] ) ? map_deep( wp_unslash( $_POST['seo_title'] ), 'sanitize_text_field' ) : array();
		$descriptions = isset( $_POST['seo_description'] ) ? map_deep( wp_unslash( $_POST['seo_description'] ), 'sanitize_textarea_field' ) : array();
		$robots       = isset( $_POST['robots_index'] ) ? map_deep( wp_unslash( $_POST['robots_index'] ), 'sanitize_text_field' ) : array();
		...
		foreach ( $post_ids as $post_id ) {
			...
			if ( isset( $robots[ $post_id ] ) && ! empty( $robots[ $post_id ] ) ) {
				$robots_value = in_array( $robots[ $post_id ], array( 'index', 'noindex' ), true ) ? $robots[ $post_id ] : 'index';
