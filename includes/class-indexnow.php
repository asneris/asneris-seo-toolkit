...
class ASNERISSEO_IndexNow {
	...
	public static function register_rewrite(): void {
		...
		add_action(
			'template_redirect',
			function () use ( $key ) {
				if ( get_query_var( 'ASNERISSEO_indexnow_keyfile' ) != 1 ) {
					return;
				}
	...
	public static function ajax_manual_submit(): void {
		...
		if ( ! $post_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid post ID', 'asneris-seo-toolkit' ) ) );
			return;
		}

		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			wp_send_json_error( array( 'message' => __( 'You do not have permission to edit this post.', 'asneris-seo-toolkit' ) ) );
			return;
		}

		$post = get_post( $post_id );
		if ( ! $post || $post->post_status !== 'publish' ) {
			wp_send_json_error( array( 'message' => __( 'Post must be published', 'asneris-seo-toolkit' ) ) );
			return;
		}

		$url = get_permalink( $post_id );
		if ( ! $url ) {
			wp_send_json_error( array( 'message' => __( 'Could not get permalink', 'asneris-seo-toolkit' ) ) );
			return;
		}
