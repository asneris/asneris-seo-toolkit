...
class ASNERISSEO_Validation {
	...
	public static function enqueue_assets( $hook ) {
		...
		if ( $hook !== 'asneris-seo-toolkit_page_' . ASNERIS_MENU_SLUG . '-validation' ) {
			return;
		}
		...
		if ( file_exists( $react_asset_path ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'mountSelector'                  => '.asnerisseo-fallback-validation',
					'hideFallback'                   => true,
				)
			);
		}
	...
	public static function validate_schema_blocks( $schema_array ) {
		...
		foreach ( $type_counts as $type => $count ) {
			if ( $count > 1 && in_array( $type, array( 'Article', 'WebPage', 'Product' ) ) ) {
	...
	public static function get_indexnow_status() {
		$settings = get_option( 'ASNERISSEO_settings', array() );
	...
	private static function prepare_validation_data() {
		...
		if ( $run_test && ! empty( $test_url ) ) {
			...
			if ( ! isset( $results['error'] ) ) {
				...
				$og_title            = isset( $results['og_tags']['og:title'] ) ? array( $results['og_tags']['og:title'] ) : array();
				$og_description      = isset( $results['og_tags']['og:description'] ) ? array( $results['og_tags']['og:description'] ) : array();
				$og_image            = isset( $results['og_tags']['og:image'] ) ? array( $results['og_tags']['og:image'] ) : array();
				$twitter_card        = isset( $results['twitter_tags']['twitter:card'] ) ? array( $results['twitter_tags']['twitter:card'] ) : array();
				$twitter_title       = isset( $results['twitter_tags']['twitter:title'] ) ? array( $results['twitter_tags']['twitter:title'] ) : array();
				$twitter_description = isset( $results['twitter_tags']['twitter:description'] ) ? array( $results['twitter_tags']['twitter:description'] ) : array();
				$twitter_image       = isset( $results['twitter_tags']['twitter:image'] ) ? array( $results['twitter_tags']['twitter:image'] ) : array();
				$google_verification = isset( $results['verification']['google'] ) ? array( $results['verification']['google'] ) : array();
				$msvalidate          = isset( $results['verification']['bing'] ) ? array( $results['verification']['bing'] ) : array();
				$yandex_verification = isset( $results['verification']['yandex'] ) ? array( $results['verification']['yandex'] ) : array();
				...
				$data['schema_check'] = ! empty( $results['schema'] ) ? self::validate_schema_blocks( $results['schema'] ) : array(
					'status'  => 'fail',
	...
	private static function calculate_overall_score( $results, $data ) {
		...
		if ( count( $results['title'] ) === 1 ) {
			++$checks['recommended']['passed'];
		}
		if ( count( $results['description'] ) === 1 ) {
			++$checks['recommended']['passed'];
		}
		if ( count( $results['canonical'] ) === 1 ) {
			++$checks['recommended']['passed'];
		}
		if ( $data['sitemap'] && isset( $data['sitemap']['status'] ) && $data['sitemap']['status'] === 'exists' ) {
			++$checks['recommended']['passed'];
		}
		if ( $data['robots'] && isset( $data['robots']['status'] ) && $data['robots']['status'] === 'exists' ) {
			++$checks['recommended']['passed'];
		}
		if ( $data['headings'] && $data['headings']['has_h1'] ) {
			++$checks['recommended']['passed'];
		}
		...
		// Social media (6)
		if ( count( $results['og_title'] ) >= 1 ) {
			++$checks['optimization']['passed'];
		}
		if ( count( $results['og_description'] ) >= 1 ) {
			++$checks['optimization']['passed'];
		}
		if ( count( $results['og_image'] ) >= 1 ) {
			++$checks['optimization']['passed'];
		}
		if ( count( $results['twitter_card'] ) >= 1 ) {
			++$checks['optimization']['passed'];
		}
		if ( count( $results['twitter_title'] ) >= 1 ) {
			++$checks['optimization']['passed'];
		}
		if ( count( $results['twitter_description'] ) >= 1 ) {
			++$checks['optimization']['passed'];
		}
		// Rich results (1)
		if ( $data['schema_check'] && $data['schema_check']['status'] === 'pass' ) {
			++$checks['optimization']['passed'];
		}
		// Content (2)
		if ( $data['images'] && $data['images']['with_alt'] > 0 ) {
			++$checks['optimization']['passed'];
		}
		if ( $data['links'] && ( $data['links']['internal'] > 0 || $data['links']['external'] > 0 ) ) {
			++$checks['optimization']['passed'];
		}
		// Verification (3)
		if ( count( $results['google_verification'] ) >= 1 ) {
			++$checks['optimization']['passed'];
		}
		if ( count( $results['msvalidate'] ) >= 1 ) {
			++$checks['optimization']['passed'];
		}
		if ( count( $results['yandex_verification'] ) >= 1 ) {
			++$checks['optimization']['passed'];
		}
		// Performance (1)
		if ( $data['indexnow'] && $data['indexnow']['configured'] ) {
			++$checks['optimization']['passed'];
		}
	...
	private static function save_validation_results( $data ) {
		...
		if ( isset( $data['results'] ) ) {
			...
			if ( isset( $data['results']['title'] ) && count( $data['results']['title'] ) !== 1 ) {
				++$conflicts;
			}
			if ( isset( $data['results']['description'] ) && count( $data['results']['description'] ) > 1 ) {
				++$warnings;
			}
			if ( isset( $data['results']['canonical'] ) && count( $data['results']['canonical'] ) !== 1 ) {
				++$conflicts;
			}

			// Check sitemap and robots
			if ( $data['sitemap'] && $data['sitemap']['status'] !== 'exists' ) {
				++$warnings;
			}
			if ( $data['robots'] && $data['robots']['status'] !== 'exists' ) {
				++$warnings;
			}
		}
	...
	private static function save_diagnostics_summary() {
		...
		if ( $sitemap_status['http_status'] !== 200 ) {
			++$conflicts;
		}
		if ( ! $sitemap_status['in_robots'] ) {
			++$warnings;
		}
	...
	<div class="wrap ASNERISSEO-admin-wrap">
		...
		<div class="asnerisseo-fallback-validation">
		<h1>
		...
		$template_dir = plugin_dir_path( __DIR__ ) . 'templates/validation/';
		...
		</div><!-- .asnerisseo-fallback-validation -->
	</div><!-- .wrap -->
