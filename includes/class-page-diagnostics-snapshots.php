...
class ASNERISSEO_Page_Diagnostics_Snapshots {
  ...
  private static $canonical_tab_field_registry = [
    'overview' => [
      'HTTP Status',
      'Robots Meta',
      'SEO Title Length',
  ...
  public static function init() {
    add_action( self::SCAN_CRON_HOOK, [ __CLASS__, 'run_priority_pages_scan_cron' ] );
  ...
  private static function table_exists( $table_name ) {
    ...
    // Use a generic probe query so table checks work on MySQL and PostgreSQL/pg4wp.
    ...
    $probe = $wpdb->get_var( "SELECT 1 FROM {$table_name} LIMIT 1" );
  ...
  public static function run_priority_pages_scan_cron( $limit = null ) {
    ...
    foreach ( array_slice( $priority_ids, 0, $max_to_scan ) as $post_id ) {
      ...
      $score_override = null;
      if ( class_exists( 'ASNERISSEO_REST_API' ) && method_exists( 'ASNERISSEO_REST_API', 'build_weightage_score_override_for_post' ) ) {
        $score_override = ASNERISSEO_REST_API::build_weightage_score_override_for_post( $post, $checks );
      }
  ...
  public static function save_snapshot( WP_Post $post, array $checks, $url, $score_override = null ) {
    ...
    $overview_run_id = '';
    $seo_score_message = '';
    $score_engine = 'fallback_check_bucket';

    if ( ! is_array( $score_override ) && class_exists( 'ASNERISSEO_REST_API' ) && method_exists( 'ASNERISSEO_REST_API', 'build_weightage_score_override_for_post' ) ) {
      $score_override = ASNERISSEO_REST_API::build_weightage_score_override_for_post( $post, $checks );
    }
    ...
    if ( is_array( $score_override ) ) {
      ...
      if ( isset( $score_override['health'] ) ) {
        ...
        if ( in_array( $override_health, [ 'good', 'warning', 'poor' ], true ) ) {
    ...
    return array_map(
      static function( $row ) {
        ...
        return [
          ...
          'issueGroups' => isset( $decoded['issueGroups'] ) && is_array( $decoded['issueGroups'] ) ? $decoded['issueGroups'] : [],
          ...
          'tabIssueRecords' => self::normalize_tab_issue_records(
            isset( $decoded['tabIssueRecords'] ) && is_array( $decoded['tabIssueRecords'] ) ? $decoded['tabIssueRecords'] : [],
            isset( $decoded['overviewIssueRecords'] ) && is_array( $decoded['overviewIssueRecords'] ) ? $decoded['overviewIssueRecords'] : [],
            isset( $decoded['aiIssueRecords'] ) && is_array( $decoded['aiIssueRecords'] ) ? $decoded['aiIssueRecords'] : [],
            isset( $decoded['checks'] ) && is_array( $decoded['checks'] ) ? $decoded['checks'] : []
          ),
          'checks' => isset( $decoded['checks'] ) && is_array( $decoded['checks'] ) ? array_values( $decoded['checks'] ) : [],
        ];
      },
      $rows
    );
  ...
  private static function with_tab_issue_records( array $report ) {
    ...
    if ( isset( $report['tabIssueRecords'] ) && is_array( $report['tabIssueRecords'] ) ) {
      $report['tabIssueRecords'] = self::normalize_tab_issue_records(
        ...
        $overview_issue_records,
        $ai_issue_records,
        isset( $report['checks'] ) && is_array( $report['checks'] ) ? $report['checks'] : []
      );
      return $report;
    }
  ...
  private static function normalize_tab_issue_records( array $tab_issue_records, array $overview_issue_records, array $ai_issue_records, array $checks = [] ) {
  ...
  private static function build_tab_issue_records( array $overview_issue_records, array $ai_issue_records, array $checks = [] ) {
  ...
  private static function normalize_check_status( $status ) {
    ...
    if ( in_array( $normalized, [ 'pass', 'warning', 'fail' ], true ) ) {
      return $normalized;
    }
  ...
  private static function build_materialized_issue_record( $tab_key, $canonical_field, array $check = null, $run_id = '' ) {
    ...
    if ( is_array( $check ) ) {
      ...
      $linked_fields[] = self::normalize_issue_field_label( (string) ( $check['label'] ?? $canonical_field ) );
    }
  ...
  private static function materialize_missing_tab_issue_records( array $tab_records, array $checks = [] ) {
  ...
  private static function derive_scores_from_checks( array $checks ) {
    ...
    foreach ( $checks as $check ) {
      ...
      if ( ! isset( $canonical_buckets[ $canonical_field ] ) ) {
        $canonical_buckets[ $canonical_field ] = [
          'sum' => 0.0,
          'count' => 0,
        ];
      }
  ...
  private static function resolve_canonical_field_from_check( array $check ) {
    ...
    $canonical_patterns = [
      'Robots Meta' => [ '/robots directives|robots meta|x-robots-tag|indexability|follow directive/' ],
      'HTTP Status' => [ '/http status|redirect status|final destination|canonical target http 200/' ],
      'SEO Title' => [ '/^seo title$|title quality|seo title length/' ],
      'Meta Description' => [ '/^meta description$|description quality|meta description length/' ],
      'H1 Heading' => [ '/h1 exists|h1 presence|h1 present|multiple h1|heading structure|heading hierarchy|content depth|content present|readability/' ],
      'Internal Links' => [ '/internal links|external links|nofollow links/' ],
    ];
  ...
  private static function derive_issue_groups_from_checks( array $checks ) {
    ...
    foreach ( $checks as $check ) {
      ...
      if ( in_array( $category, [ 'search', 'social', 'schema' ], true ) || preg_match( '/title|meta|description|open graph|twitter/', $label ) ) {
        $groups['meta'] = true;
      }
      ...
      if ( in_array( $category, [ 'quality', 'links' ], true ) || preg_match( '/word count|h1|h2|image|alt|link/', $label ) ) {
        $groups['content'] = true;
      }
