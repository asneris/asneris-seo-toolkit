...
class ASNERISSEO_Redirects {
	...
	public static function enqueue_assets( $hook ) {
		...
		if ( $hook !== 'asneris-seo-toolkit_page_' . ASNERIS_MENU_SLUG . '-redirects' ) {
			return;
		}
		...
		if ( file_exists( $react_asset_path ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'mountSelector'                  => '.asnerisseo-fallback-redirects',
					'hideFallback'                   => true,
				)
			);
		}
	...
	public static function track_slug_change( $post_id, $post_after, $post_before ) {
		...
		// Only create auto redirects when an already-published URL changes.
		...
		if ( $redirect_map === null ) {
			...
			foreach ( $redirects as $redirect ) {
				...
				if ( ! empty( $from_query ) ) {
					...
					$redirect_map_with_query[ $key ] = array(
						'to'   => $redirect['to'],
						'code' => (int) $redirect['code'],
					);
				} else {
					// Store path-only redirects
					$redirect_map[ $from_path ] = array(
						'to'   => $redirect['to'],
						'code' => (int) $redirect['code'],
					);
				}
	...
	public static function add_redirect( $from, $to, $code = 301, $type = 'manual' ) {
		...
		if ( count( $redirects ) >= 500 ) {
			if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
				wp_send_json_error( array( 'message' => 'Redirect limit reached (500 maximum). Please delete some redirects before adding new ones.' ) );
			}
			return false;
		}
		...
		$redirects = array_filter(
			$redirects,
			function ( $r ) use ( $from_normalized ) {
	...
	public static function render_page() {
		...
		if ( isset( $_POST['ASNERISSEO_add_redirect'] ) && check_admin_referer( 'ASNERISSEO_redirect_add' ) ) {
			...
			$code = in_array( $code, array( 301, 302, 307 ), true ) ? $code : 301;
	...
	<div class="wrap ASNERISSEO-admin-wrap">
		...
		<div class="asnerisseo-fallback-redirects">
		<h1>
		...
		</div><!-- .asnerisseo-fallback-redirects -->
	</div>
