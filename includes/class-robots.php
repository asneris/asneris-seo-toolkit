...
class ASNERISSEO_Robots {
	...
	public static function init() {
		...
		add_action( 'admin_post_ASNERISSEO_save_robots', array( __CLASS__, 'save_robots' ) );
	...
	public static function validate() {
		...
		$file_exists = file_exists( self::$robots_file );
		$http_ok     = false;
		...
		// Check 2: robots.txt availability (physical or virtual)
		$is_available                = $file_exists || $http_ok;
		$results['checks']['exists'] = array(
			'status'  => $is_available ? 'pass' : 'fail',
			'label'   => 'robots.txt file',
			'message' => $is_available
				? ( $file_exists ? 'robots.txt file found' : 'robots.txt is served virtually' )
				: 'Not found on your site',
		);

		if ( ! $is_available && $results['status'] !== 'error' ) {
			$results['status']     = 'warning';
			$results['warnings'][] = 'No robots.txt file found. Search engines will crawl all accessible pages by default.';
		}

		// Get content (filesystem fallback only when HTTP body is unavailable)
		global $wp_filesystem;
		if ( empty( $wp_filesystem ) ) {
			require_once ABSPATH . 'wp-admin/includes/file.php';
			WP_Filesystem();
		}
		if ( ! $has_content && $file_exists ) {
		...
		if ( $has_content ) {
			...
			$results['checks']['sitewide_block'] = array(
		...
		if ( $has_content ) {
			...
			$results['checks']['assets_allowed'] = array(
		...
		if ( $has_content ) {
			...
			$results['checks']['sitemap_declared'] = array(
		...
		if ( $has_sitemap && '' !== $sitemap_url ) {
			...
			if ( is_wp_error( $sitemap_response ) ) {
				$results['checks']['sitemap_reachable'] = array(
					'status'  => 'fail',
		...
		if ( $has_content ) {
			...
			$results['checks']['no_conflicts'] = array(
		...
		foreach ( $lines as $line ) {
			...
			if ( stripos( $line, 'User-agent:' ) === 0 ) {
				...
				if ( ! isset( $rules_by_agent[ $current_user_agent ] ) ) {
					$rules_by_agent[ $current_user_agent ] = array(
						'allow'    => array(),
						'disallow' => array(),
					);
				}
